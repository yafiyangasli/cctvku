	...
	<div class="col-10 offset-1 offset-sm-1 col-sm-7 order-sm-1 col-md-7 order-md-1 pb-5">
		...
		<?php foreach($cart as $ct):?>
			<?php foreach($produk as $pr):?>
				<?php if($ct['id_produk']==$pr['id_produk']):?>
					  <li class="media pb-5">
					  	<a href="<?=base_url('catalogue/detailProduk/').$pr['id_produk'];?>" class="text-dark text-decoration-none">
					    <img src="<?php echo base_url();?>assets/images/cctv/<?php echo $pr['gambar'];?>" alt="<?=$pr['nama'];?>" style="max-width: 150px; min-width: 100px;" class="img-fluid order-sm-1 mr-2"></a>
					    <div class="media-body order-sm-2">
					    <div class="row no-gutters">
					    	<div class="col-12 col-md-5">
						      <a href="<?=base_url('catalogue/detailProduk/').$pr['id_produk'];?>" class="text-dark text-decoration-none"><h5 class="mt-0 mb-1"><?=$pr['nama'];?></h5></a>
					    	</div>
					    	...
					    	<form method="post" action="<?=base_url('user/updateCart/').$ct['id_cart'];?>">
					    		...
					    		<div class="form-inline" style="padding-top: 0.5rem;">
									...
									<?php if ($pr['stok'] >= $ct['jumlah']):?>
										<option value="<?= $ct['jumlah']?>"><?= $ct['jumlah']?></option>
									<?php elseif($pr['stok'] > 0):?>
										<?php redirect('user/updateCartStokKurang/'.$ct['id_cart'].'/'.$pr['stok']);?>
									<?php elseif($pr['stok'] <=0):?>
